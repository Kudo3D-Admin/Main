...
class PrintimgThread (threading.Thread):
	...
	def __init__(self, thread_id, name, thread_lock, sendCommand, setjobCommand, sendlogCommand, finish_print):
		...
		self.black = (0,0,0)
		self.blankimg = pygame.image.load('/home/pi/Blank_1920x1080.png')
		self.gridimg = pygame.image.load('/home/pi/Grid_1920x1080.png')
...
#delay = delay - 0.5
			...
			time.sleep(db)
			self.sendCommand("M280 P0 S90")
			self.sendlogCommand("G1 Z 100")
			self.sendCommand("G91")
			self.sendCommand("G1 Z100 F150")
			self.sendCommand("G90")
			self.sendCommand("M114")
			self.sendlogCommand("Power off Projector")
			if self.pro == "OPTOMA":
				self.sendCommand("M101")
			elif self.pro == "acer_IR":
				self.sendCommand("M103")
			elif self.pro == "viewsonic_IR":
				self.sendCommand("M99")
			else:
				port1 = None
				port1 = self.port1
				with open('/home/pi/' + self.pro, "rb") as ins:
					array = []
					for line in ins:
						array.append(line)
				if array[0]:
		...
		if self.spectype == "LayerGroup":
			...
			for i in range(len(self.data)):
				...
				if remaining_layers > 0:
					...
					time_per_layer = exposure + (lift / upspeed + (lift - thickness) / downspeed) * 60 + delay
...
class Printer(PrinterInterface, comm.MachineComPrintCallback):
	...
	# ~~ handling of PrinterCallbacks
		...
		self.display_frame.eshutter = settings().getBoolean(["printerset", "shutter"])
		self.display_frame.emask = settings().getBoolean(["printerset", "mask"])
	...
	def jog(self, axis, amount):
		...
		movement_speed = settings().getInt(["printerset", "zrate"]) #printer_profile["axes"][axis]["speed"]
